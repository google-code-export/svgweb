...
(function(){ // hide everything externally to avoid name collisions
  ...
  FlashSVG.prototype.onFlashLoaded = function(svgID) {
    // the Flash object is done loading
    console.log('onFlashLoaded, svgID='+svgID);
  ...
  FlashSVG.prototype.onRenderingFinished = function(svgID) {
    var i;
    
    console.log('onRenderingFinished, svgID='+svgID);
    ...
    if (!this._pageLoadFinished) {
      ...
      if (this._svgLoaded == document.getElementsByTagName('svg').length) {
        ...
        var rootNodes = document.getElementsByTagName('svg');
        for (i = 0; i < rootNodes.length; i++) {
          var onload = rootNodes[i].onload;
          if (onload) {
            eval(onload);
          }
        }
  ...
  FlashSVG.prototype.debug = function(msg) {
  ...
  FlashSVG.prototype._exportFlashMethods = function() {
    ...
    // must be global
    var self = this;
    window.__svg__onFlashLoaded = function(svgID) {
      self.onFlashLoaded(svgID);
    }
    window.__svg__onRenderingFinished = function(svgID) {
      self.onRenderingFinished(svgID);
    }
    window.__svg__debug = this.debug;
  }
