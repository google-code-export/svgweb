...
function runTests(embedTypes) {
  ...
  obj.addEventListener('load', function() {
    ...
    assertEquals('this == SVG OBJECT', obj, this);
  ...
  obj.addEventListener('load', function() {
    ...
    obj = matches[5];
    assertExists('SVG OBJECT should exist', obj);
    
    // check autogenerated ID
    if (renderer == 'native') {
      assertEquals('SVG OBJECT should have no id', '', obj.id);
    } else {
      assertExists('SVG OBJECT should have an ID', obj.getAttribute('id'));
      assertTrue('SVG OBJECT should have randomly generated ID',
                 obj.getAttribute('id').indexOf('__svg__random') != -1);
    } 
